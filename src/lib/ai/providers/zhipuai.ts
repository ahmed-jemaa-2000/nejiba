 ...
 *
 * Implements the AIProvider interface using ZhipuAI's GLM-4.6 model.
 * Uses OpenAI SDK compatibility with Z.AI API endpoint.
 */
...
export class ZhipuAIProvider implements AIProvider {
    ...
    constructor() {
        if (!process.env.ZHIPUAI_API_KEY) {
            throw new InvalidAPIKeyError("zhipuai");
        }

        // Initialize OpenAI client with ZhipuAI base URL
        this.client = new OpenAI({
            apiKey: process.env.ZHIPUAI_API_KEY,
            baseURL: "https://api.z.ai/api/paas/v4/",
        });

        // Use GLM-4.6 for ALL tasks (single model approach)
        this.config = {
            workshop: "glm-4.6",
            activities: "glm-4.6",
...
Generate workshop plan for "${input.topic}" now.`;
        ...
        console.log("🎓 [GLM-4.6] Generating workshop for:", input.topic, "| Duration:", durationNum, "min | Age:", input.ageRange);
        ...
        try {
            const completion = await this.client.chat.completions.create({
                ...
                max_tokens: this.config.maxTokensWorkshop,
            ...
            if (!content) {
                throw new AIError("No response from ZhipuAI", "NO_RESPONSE", "zhipuai");
            }
...
Each post must include surprising facts, research-backed statistics, and actionable tips.`;
        ...
        try {
            ...
            console.log("\n" + "=".repeat(60));
            console.log("📦 [GLM-4.6] Generated", tips.length, "daily tips for:", topic);
...
Style: High-end 3D Pixar Style, set in a bright Tunisian cultural club.`;
    ...
    private parseWorkshopPlan(content: string): WorkshopPlanData {
        try {
            ...
            console.log("✅ [GLM-4.6] Workshop plan generated successfully with", parsed.timeline?.length || 0, "activities");
            return parsed;
        } catch (parseError) {
            ...
            throw new AIError(`Failed to parse workshop plan JSON: ${parseError}`, "PARSE_ERROR", "zhipuai");
        }
    }
    ...
    private parseJSON<T>(content: string): T {
        try {
            ...
            throw new AIError("Failed to parse JSON", "PARSE_ERROR", "zhipuai");
        }
    }
    ...
    private handleError(error: any): never {
        ...
        if (error.status === 429) {
            throw new RateLimitError("zhipuai");
        }
        throw new AIError(message, "GENERATION_ERROR", "zhipuai", error.status || 500);
    }
}
