...
export interface PDFReadyActivity {
  ...
  // === CLARITY ENHANCEMENTS (for best results) ===

  // Challenge Progression (for different skill levels)
  challengeProgression?: {
    level1: string;           // Basic task (all kids can do)
    level2: string;           // Medium challenge (most kids)
    level3: string;           // Advanced challenge (fast finishers)
    facilitatorPhrases: {
      forLevel2: string;      // "هل تريد تحدي أكبر؟"
      forLevel3: string;      // "للأبطال الخارقين: جرب هذا!"
    };
  };

  // Observable Impact Indicators
  impactIndicators?: {
    lookFor: {
      emerging: string;       // "الطفل يحاول لكن يحتاج مساعدة"
      developing: string;     // "الطفل يكمل بمساعدة قليلة"
      proficient: string;     // "الطفل يكمل باستقلالية"
      excelling: string;      // "الطفل يساعد الآخرين"
    };
    quickCheck: string[];     // Quick facilitator checklist
  };

  // Multi-Sensory Engagement
  sensoryEngagement?: {
    visual: string;           // What kids SEE
    tactile: string;          // What kids TOUCH
    auditory: string;         // What kids HEAR
    kinesthetic: string;      // What kids DO with their body
  };
}
...
# 🌟 معايير الوضوح والجودة العالية (للحصول على أفضل النتائج)

## الخطوات الواضحة:
- **فعل واحد فقط لكل خطوة**: "ارفع الكوب" ✅ وليس "ارفع الكوب وزينه واكتب عليه" ❌
- **جملة من 5-8 كلمات كحد أقصى**: أبسط = أفضل
- **العبارة المنطوقة جاهزة للقراءة**: الميسر يقرأها مباشرة

## التحدي التدريجي (للأنشطة الإبداعية):
- **المستوى 1**: مهمة أساسية ينجزها كل طفل
- **المستوى 2**: تحدي إضافي للمتقدمين (عبارة: "هل تريد تحدي أكبر؟")
- **المستوى 3**: مهمة متقدمة للسريعين (عبارة: "للأبطال الخارقين!")

## التنوع الحسي (على الأقل حاستين لكل نشاط):
- 👁️ **البصر**: ألوان زاهية، أشكال واضحة
- ✋ **اللمس**: ملمس مختلف (ناعم، خشن، لزج)
- 👂 **السمع**: موسيقى، تصفيق، أصوات
- 🏃 **الحركة**: قفز، مشي، توازن

## مؤشرات التأثير الملموس:
اكتب ما يمكن للميسر ملاحظته:
- "الطفل يبتسم عند الانتهاء"
- "الطفل يشارك مع زميله بدون تشجيع"
- "الطفل يقول 'أريد أن أحاول مرة أخرى'"
